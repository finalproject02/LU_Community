                                ...
                                <Col md="5" className="pt-2">
                                    <div className="d-flex justify-content-between align-items-center mt-2 mt-sm-0">
                                        <div className="d-flex justify-content-start align-items-center">
                                            <img onClick={() => action(item.owner_id, item.owner_position, item.post_to)} src={getUserProfilePicture(item.owner_id, item.owner_position, item.post_to) !== null ? `/api/files/storage/${getUserProfilePicture(item.owner_id, item.owner_position, item.post_to)}` : Avatar} alt="name" width="60" height="60" className="rounded-circle me-2" />
                                            <div>
                                    ...
                                    <div className="d-flex align-items-center justify-content-between">
                                        ...
                                        <div className="d-flex align-items-center py-2">
                                            <small className="anchor text-dark textHover pe-3 cursor"><span>{item.comments.length}</span> comment</small>
                                        </div>
                                    </div>
                                    <hr className="hr" />
                                    ...
                                    <div className="mt-2">
                                        ...
                                        {!showMore && item.comments.sort((a, b) => { return new Date(b.time) - new Date(a.time) }).slice(0, 3).map(comment => (
                                            <div className="d-flex justify-content-start align-items-start ms-5 mb-2">
                                                <img onClick={() => action(comment.id, 'own_post', '')} width="35" height="35" className="rounded-circle me-2 cursor" src={getUserProfilePicture(comment.id, 'own_post', '') ? `/api/files/storage/${getUserProfilePicture(comment.id, 'own_post', '')}` : Avatar} alt="" />
                                                <div className="commentBg rounded-3 p-2">
                                                    <h6 className="cursor textHover" onClick={() => action(comment.id, 'own_post', '')}>{getUserName(comment.id, 'own_post', '')}</h6>
                                                ...
                                                <NavDropdown
                                                    ...
                                                    <NavDropdown.Item className="dropdownItem py-1">Edit comment
                                                    </NavDropdown.Item>
                                                </NavDropdown>
                                            </div>
                                        ))}
                                        ...
                                        {showMore && item.comments.sort((a, b) => { return new Date(b.time) - new Date(a.time) }).map(comment => (
                                            <div className="d-flex justify-content-start align-items-start ms-5 mb-2">
                                                ...
                                                <div className="commentBg rounded p-2">
                                                    <h6 className="cursor textHover">{getUserName(comment.id, 'own_post', '')}</h6>
                                                    <p className="mb-0">{comment.comment}</p>
                                                </div>
                                                <NavDropdown
                                                    className="navFontSize"
