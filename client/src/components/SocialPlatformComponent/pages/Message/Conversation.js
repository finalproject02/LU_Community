...
const Conversation = ({ currentPeer }) => {
    ...
    return (
        <div>
            <div className="d-flex justify-content-between align-items-center profileHeader shadow">
                {peerProfile.map(profile => (
                    <div className="d-flex profileInfo">
                        <div>
                            <img width="45" height="45" className="rounded-circle cursor" src={profile?.profile_picture ? `/api/files/storage/${profile?.profile_picture}` : Avatar} alt=".." onClick={handleShow} />
            ...
            <div className="messageContainer p-4 chat">
