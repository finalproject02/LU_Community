            ...
            {currentUser?.connection_requests.length !== 0 &&(
                <Card className="w-100">
                    <Card.Body>
                        ...
                        <Card.Text as="div">
                            <Row>
                                {!showMore && currentUser?.connection_requests.slice(0, 4).sort((a, b) => {return new Date(b.time) - new Date(a.time)}).map(requests => (

                                    <Col key={requests.userId} md="3" className=" mb-2">
                                        <Card className="w-100" style={{ cursor: 'pointer' }}>
                                            <Card.Img src={getUserProfilePicture(requests.userId) !== null ? `/api/files/storage/${getUserProfilePicture(requests.userId)}` : Avatar} style={{ cursor: 'pointer', height: "20vh" }} className="card-img-top rounded-3 w-100" alt="name" onClick={() => { history.push(`/profile/${requests.userId}`) }} />
                                            <Card.Body>
                                                <Card.Title onClick={() => { history.push(`/profile/${requests.userId}`) }}>{getUserName(requests.userId)}</Card.Title>
                                                <Card.Text as="div" className="mt-2">
                                                    <div className="bgSecondary text-center rounded-3 mb-2">
                                                        <Link to="#" className="btn w-100 text-white" onClick={() => {
                                                            dispatch(Accept_Connection_Request(requests.userId));
                                                        }
                                                    ...
                                                    <div className="bg-secondary text-center rounded-3">
                                                        <Link to="#" className="btn w-100 text-white" onClick={() => {
                                                            dispatch(Disconnect(requests.userId));
                                                        }}>Ignore</Link>
                                                    </div>
                                                </Card.Text>
                                            </Card.Body>
                                            <Card.Footer>
                                                <small className="text-muted"> <span></span> {moment(requests.time).fromNow()}</small>
                                            </Card.Footer>
                                        </Card>
                                    </Col>
                                ...
                                {showMore && currentUser?.connection_requests.sort((a, b) => {return new Date(b.time) - new Date(a.time)}).map(requests => (
