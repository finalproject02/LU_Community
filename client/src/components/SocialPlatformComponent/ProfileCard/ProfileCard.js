                                ...
                                <Container>
                                    <Card.Body className="pt-0">
                                        <label htmlFor="coverPhoto" className="w-100">
                                            {
                                                coverPhoto && coverPhoto.type.startsWith("image") ? (<><Card.Img className="w-100 img-fluid" src={URL.createObjectURL(coverPhoto)} alt="name" style={{ objectFit: "cover" }} /><button onClick={uploadCover}>Upload</button></>) : (<Card.Img className="w-100 img-fluid" src={paramUser.length !== 0 ? paramUser.map(user => user.cover_picture ? `/api/files/storage/${user.cover_picture}` : Avatar) : currentUser?.cover_picture ? `/api/files/storage/${currentUser?.cover_picture}` : Avatar} alt={'Not found'} style={{ height: "20rem", objectFit:'cover' }} />)
                                            }
                                            {paramUser.length === 0 && (
                                        ...
                                        <Row className="mb-2 position-relative">
                                            <label htmlFor="profilePhoto" className="userPosition">
                                                {
                                                    profilePhoto && profilePhoto.type.startsWith("image") ? (<><Card.Img style={{ objectFit: 'cover' }} className="img-fluid w-25 rounded-pill p-2 bg-light" src={URL.createObjectURL(profilePhoto)} alt="name" /><button onClick={uploadProfile}>Upload</button></>) : (<img src={paramUser.length !== 0 ? paramUser.map(user => user.profile_picture ? `/api/files/storage/${user.profile_picture}` : Avatar) : currentUser?.profile_picture ? `/api/files/storage/${currentUser?.profile_picture}` : Avatar} className="img-fluid userProfile p-2 bg-light" alt={'Not Found'} style={{ height: '150px', objectFit: 'scalable' }} />)
                                                }
                                                {paramUser.length === 0  && (
